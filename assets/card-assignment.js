...
(() => {
  ...
  async function loadData() {
    const [{data: cs, error: ce}, {data: us, error: ue}] = await Promise.all([
      sb.from('Cards').select('id,card_code,status,customer_id,product_type,google_review_url,activated_at,expires_at').order('created_at',{ascending:false}),
  ...
  function render() {
    ...
    $('assignCount').textContent = `${unassigned.length} kartu tersedia • ${customers.length} customer`;
  }
  async function assign() {
    ...
    const {data,error} = await sb.from('Cards').update({customer_id:customerId, google_review_url:customer?.google_review_url || null}).eq('id',cardId).is('customer_id',null).select('id,card_code,customer_id');
    if (error) return msg('err',`❌ Gagal assign kartu: ${error.message}`);
    if (!data?.length) return msg('err','❌ Kartu sudah di-assign atau tidak ditemukan. Muat ulang data.');
  ...
  function init(){ if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',inject,{once:true}); else inject(); document.addEventListener('aljava:cards-created',()=>loadData().catch(()=>{})); document.addEventListener('aljava:customers-changed',()=>loadData().catch(()=>{})); }
