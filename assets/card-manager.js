...
(() => {
  ...
  async function autoDownloadQrZip(rows, msg) {
    ...
    for (let start = 0; start < rows.length; start += 10) {
      ...
      if (msg) message(msg,'info',`Membuat ZIP QR... ${Math.min(start+batch.length,rows.length)}/${rows.length}`);
    }
    ...
    if (msg) message(msg, failed.length ? 'info' : 'ok', `✓ ${files.length} QR berhasil dikumpulkan ke QR-KARTU-${stamp}.zip${failed.length ? ` • ${failed.length} QR gagal diunduh` : ''}`);
  }
