...
(() => {
  ...
  async function loadData() {
    ...
    return { transactions:txResult.data||[], products:Object.fromEntries((productsResult.data||[]).map(r=>[r.id,r])), customers:Object.fromEntries((customersResult.data||[]).map(r=>[r.id,r])) };
  }
  ...
  async function render(data) {
    ...
    tx.forEach(row=>{
      ...
      item.qty+=qty; item.revenue+=revenue; item.hpp+=hpp; item.commission+=commission; item.transactions++; item.paid+=paid; item.receivable+=Math.max(0,revenue-paid); byCustomer[customerKey]=item;
      ...
      statusItem.transactions++; statusItem.revenue+=revenue; statusItem.paid+=paid; statusItem.receivable+=Math.max(0,revenue-paid); byStatus[status]=statusItem;
    });
    const customerRows=Object.values(byCustomer).sort((a,b)=>b.revenue-a.revenue), statusRows=Object.values(byStatus).sort((a,b)=>b.revenue-a.revenue);
    ...
    const paidRevenue=tx.reduce((s,r)=>{const rev=Number(r.selling_price||0)*Number(r.quantity||1),paid=Math.min(Math.max(Number(r.amount_paid||0),0),rev);return s+paid;},0);
    const receivable=Math.max(0,totalRevenue-paidRevenue), unpaidTx=tx.filter(r=>{const rev=Number(r.selling_price||0)*Number(r.quantity||1),paid=Math.min(Math.max(Number(r.amount_paid||0),0),rev);return paid<rev;}).length;
