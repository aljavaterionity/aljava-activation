...
(() => {
  ...
  async function loadSales() {
    ...
    try {
      ...
      $('salesTable').innerHTML = `<div class="notice err">❌ Gagal memuat data: ${esc(error?.message || error)}</div>`;
  ...
  async function boot() {
    $('staffLoginBtn')?.addEventListener('click', login);
    $('staffPassword')?.addEventListener('keydown', (e) => { if (e.key === 'Enter') void login(); });
    $('staffLogout')?.addEventListener('click', () => void logout());
    ...
    sb.auth.onAuthStateChange((event, session) => { if (event === 'SIGNED_OUT') setLoggedOut(); else if (session?.user) setLoggedIn(session.user); });
    const { data: { session } = {} } = await sb.auth.getSession();
    if (session?.user) { setLoggedIn(session.user); await loadSales(); } else setLoggedOut();
  }
