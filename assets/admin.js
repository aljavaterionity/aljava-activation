...
(() => {
  ...
  if (!CONFIG || !sb) {
    ...
    throw new Error('ALJAVA core/Supabase client tidak tersedia.');
  }
  ...
  function showView(viewId) {
    ...
    const menuId = {
      dashboardView: 'dashboardMenu', cardsView: 'cardsMenu', productView: 'productMenu', customersView: 'customerMenu',
      salesView: 'salesMenu', operationsView: 'operationsMenu', analyticsView: 'analyticsMenu'
    }[viewId];
    if (menuId) $(menuId)?.classList.add('active');
    closeMenu();
    window.scrollTo({ top: 0, behavior: 'smooth' });
    const hash = viewId.replace(/View$/, '').replace(/^product$/, 'products');
  ...
  function bindNavigation() {
    ...
    on('resetMenu', 'click', (event) => { event.preventDefault(); event.stopPropagation(); window.__resetDashboard?.(event); });
  ...
  async function login() {
    ...
    if (!email || !password) {
      if ($('loginMsg')) $('loginMsg').innerHTML = '<div class="notice err">Email dan password wajib diisi.</div>';
      return;
    }
    ...
    try {
      ...
      if ($('loginMsg')) $('loginMsg').innerHTML = `<div class="notice err">❌ ${esc(error.message)}</div>`;
  ...
  async function load() {
    ...
    try {
      ...
      results.forEach(([key, data, error]) => {
        ...
        if (error && $(host)) $(host).innerHTML = key === 'customers'
          ? `<tr><td colspan="6"><div class="notice err">❌ ${esc(error.message)}</div></td></tr>`
  ...
  function renderStats() {
    ...
    $('activeCards').textContent = String(state.cards.filter((card) => cardStatus(card) === 'Aktif').length);
  ...
  function renderScans() {
    const counts = {};
    state.scans.forEach((scan) => { const key = scan.card_code || scan.card_id || '-'; counts[key] = (counts[key] || 0) + 1; });
  ...
  function bindUi() {
    ...
    $('password')?.addEventListener('keydown', (event) => { if (event.key === 'Enter') void login(); });
    $('refreshMenu')?.addEventListener('click', async (event) => {
      event.preventDefault(); event.stopPropagation();
      const button = $('refreshMenu'); if (!button || button.dataset.refreshBound === '1') return;
      button.dataset.refreshBound = '1'; button.disabled = true;
      try { await load(); } finally { button.disabled = false; button.dataset.refreshBound = '0'; }
    });
    ...
    sb.auth.onAuthStateChange((event) => { if (event === 'SIGNED_OUT') setLoggedOut(); });
  }
  ...
  window.adminApi = Object.freeze({ load, goDashboard, showView, getState: () => state });
  bindNavigation();
  bindUi();
  ...
  })().catch((error) => setLoggedOut(error.message || 'Gagal memulai dashboard admin.'));
})();
