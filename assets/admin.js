...
function bindNavigation() {
  ...
  $('addAccountMenu')?.addEventListener('click', (event) => {
    event.preventDefault();
    event.stopPropagation();
    openAddAccount();
  });
...
async function ensureAdmin() {
  try {
    ...
    // IMPORTANT: use the returned error variable consistently.
    // The previous code referenced an undefined `adminError` variable.
...
function renderCards() {
  ...
  const rows = state.cards.filter((card) => {
    ...
    const filterStatus = status === 'Aktif' ? 'active' : status === 'Belum Aktif' ? 'pending' : 'expired';
    ...
    return (!statusFilter || filterStatus === statusFilter) && (!search || text.includes(search));
  });
...
function prepareCustomerRows() {
  ...
  state.customerRows = state.customers.map((customer) => {
    ...
    const scans = cards.reduce((sum, card) => sum + (scansByCard[card.id] || 0), 0);
    const types = [...new Set(cards.map((card) => card.product_type).filter(Boolean))].join(', ') || '-';
    const url = customer.google_review_url || cards.find((card) => card.google_review_url)?.google_review_url || '';
    return { id: customer.id, business_name: customer.business_name || customer.owner_name || '-', product_type: types, status: customerStatus(cards), scans, url };
  });
}

function customerStatus(cards) {
  if (cards.some((card) => cardStatus(card) === 'Aktif')) return 'Aktif';
  if (cards.some((card) => cardStatus(card) === 'Belum Aktif')) return 'Pending';
...
function renderCustomers() {
  ...
  const rows = state.customerRows.filter((customer) => !search || customer.business_name.toLowerCase().includes(search) || String(customer.id).toLowerCase().includes(search));
...
$('bulkForm')?.addEventListener('submit', async (event) => {
  event.preventDefault();
  const codes = [...new Set($('bulkCodes').value.split(/\r?\n/).map((value) => value.trim()).filter(Boolean))];
  if (!codes.length) return;
  try {
    const productId = $('bulkProduct').value || null;
    const customerId = $('bulkCustomer').value || null;
    const product = state.products.find((row) => row.id === productId);
    await createCards(codes.map((cardCode) => ({ card_code: cardCode, product_type: product?.category || product?.name || 'Tanpa Jenis', status: 'pending', product_id: productId, customer_id: customerId })), $('bulkMsg'));
    event.target.reset();
  } catch (error) {
