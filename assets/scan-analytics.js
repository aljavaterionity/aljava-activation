...
(() => {
  ...
  function show(){install();document.querySelectorAll('.view').forEach(v=>v.classList.toggle('active-view',v.id==='analyticsView'));$('menuPanel')?.classList.remove('open');history.replaceState?.(null,'','#analytics');window.scrollTo({top:0,behavior:'smooth'});void load();}
  async function load(){if(loading)return;const db=client();if(!db)return;loading=true;try{
    const {data,error}=await db.from('CardScans').select('card_id,card_code,event_type,scanned_at').order('scanned_at',{ascending:false}).limit(1000); if(error)throw error; const rows=data||[], now=Date.now(), cutoff=now-30*86400000;
