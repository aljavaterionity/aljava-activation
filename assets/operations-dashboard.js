...
(() => {
  ...
  async function load(){
    ...
    try{
      ...
      const checks=[tx,customers,cards,products,scans];
      const failed=checks.find(r=>r.error); if(failed) throw failed.error;
      ...
      const overdue=unpaid.filter(r=>r.due_date && new Date(r.due_date).getTime()<today.getTime());
      $('opsActions').innerHTML=`<table><thead><tr><th>Status</th><th>Jumlah</th><th>Nominal</th></tr></thead><tbody><tr><td>🟡 Kartu pending</td><td>${pendingCards}</td><td>—</td></tr><tr><td>🟡 Transaksi belum lunas</td><td>${unpaid.length}</td><td>${money(unpaid.reduce((s,r)=>s+Math.max(0,Number(r.selling_price||0)*Number(r.quantity||1)-Number(r.amount_paid||0)),0))}</td></tr><tr><td>🔴 Jatuh tempo</td><td>${overdue.length}</td><td>${money(overdue.reduce((s,r)=>s+Math.max(0,Number(r.selling_price||0)*Number(r.quantity||1)-Number(r.amount_paid||0)),0))}</td></tr></tbody></table>`;
