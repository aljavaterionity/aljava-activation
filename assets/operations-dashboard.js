...
(() => {
  ...
  function installDarkTheme() {
    ...
    link.rel = 'stylesheet';
    link.href = '/assets/operations-dashboard-dark.css?v=20260907-010';
    document.head.appendChild(link);
  }

  function statusMarkup(label, type = 'success') {
    const safeType = type === 'warning' ? 'warning' : type === 'danger' ? 'danger' : '';
    return `<span class="ops-status${safeType ? ` ${safeType}` : ''}"><span class="ops-status-dot" aria-hidden="true"></span><span>${esc(label)}</span></span>`;
  }
  ...
  async function load() {
    ...
    try {
      ...
      $('opsActions').innerHTML = `<table><thead><tr><th>Komponen</th><th>Status</th><th>Jumlah</th></tr></thead><tbody><tr><td>Kartu</td><td>${statusMarkup(pendingCards ? 'Perlu aktivasi' : 'Normal', pendingCards ? 'warning' : 'success')}</td><td>${pendingCards} pending</td></tr><tr><td>Transaksi</td><td>${statusMarkup('Otomatis')}</td><td>${rows.length} transaksi</td></tr><tr><td>Pembayaran</td><td>${statusMarkup(unpaid ? 'Ada transaksi belum lunas' : 'Lunas', unpaid ? 'warning' : 'success')}</td><td>${unpaid} belum lunas</td></tr><tr><td>Revenue</td><td>${statusMarkup('Otomatis')}</td><td>${money(revenue)}</td></tr></tbody></table>`;
